...
export default function LandingPage() {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-indigo-50 font-sans">
            ...
            <nav className="flex items-center justify-between px-4 sm:px-8 md:px-12 py-4 bg-white border-b border-indigo-100">
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                    <button
                        ...
                        className="px-3 sm:px-5 py-2 border border-indigo-600 text-indigo-600 rounded-lg text-xs sm:text-sm font-semibold cursor-pointer hover:bg-indigo-50 transition"
                    >
                        Sign in
                    </button>
                    <button
                        onClick={() => navigate("/register")}
                        className="px-3 sm:px-5 py-2 bg-indigo-600 text-white rounded-lg text-xs sm:text-sm font-semibold cursor-pointer hover:bg-indigo-700 transition"
                    >
                        Get started
                    </button>
                </div>
            ...
            {/* ── Hero ── */}
            <section className="flex flex-col lg:flex-row items-center justify-between px-4 sm:px-8 md:px-12 py-10 sm:py-14 md:py-20 gap-8 md:gap-12 bg-white">
                {/* Left */}
                <div className="flex-1 max-w-xl w-full text-center lg:text-left">
                    ...
                    <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold leading-tight text-indigo-950 mb-5">
                        Your library,{" "}
                    ...
                    <div className="flex flex-wrap justify-center lg:justify-start gap-3">
                        <button
                            onClick={() => navigate("/register")}
                            className="px-5 sm:px-7 py-3 bg-indigo-600 text-white rounded-xl text-sm sm:text-base font-bold cursor-pointer hover:bg-indigo-700 active:scale-[0.98] transition"
                        >
                            Create free account
                        </button>
                        <button
                            onClick={() => navigate("/login")}
                            className="px-5 sm:px-7 py-3 border-2 border-indigo-600 text-indigo-600 rounded-xl text-sm sm:text-base font-bold cursor-pointer hover:bg-indigo-50 active:scale-[0.98] transition"
                        >
                            Sign in →
                        </button>
                    </div>
                ...
                <div className="w-full sm:w-96 lg:w-80 shrink-0 flex flex-col gap-3">
                    {/* Total books card */}
            ...
            {/* ── Stats strip ── */}
            <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-indigo-100 border-y border-indigo-100 bg-white">
                {stats.map((s) => (
                    <div key={s.label} className="py-6 sm:py-7 text-center">
                        <p className="text-2xl sm:text-3xl font-extrabold text-indigo-600">{s.num}</p>
            ...
            {/* ── Features ── */}
            <section className="px-4 sm:px-8 md:px-12 py-12 sm:py-16 bg-indigo-50">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
                    {features.map((f) => (
                        <div key={f.title} className="bg-white border border-indigo-100 rounded-2xl p-5 sm:p-6">
            ...
            {/* ── CTA Banner ── */}
            <section className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 sm:px-8 md:px-12 py-10 sm:py-14 bg-indigo-600 gap-6 sm:gap-10">
                <div>
                    <h2 className="text-xl sm:text-2xl font-extrabold text-white max-w-md leading-snug">
                ...
                <div className="flex flex-wrap gap-3 shrink-0 w-full sm:w-auto">
                    <button
                        onClick={() => navigate("/register")}
                        className="flex-1 sm:flex-none px-5 sm:px-6 py-3 bg-white text-indigo-600 rounded-xl text-sm font-bold cursor-pointer hover:bg-indigo-50 transition text-center"
                    >
                        Create free account
                    </button>
                    <button
                        onClick={() => navigate("/login")}
                        className="flex-1 sm:flex-none px-5 sm:px-6 py-3 border border-white/50 text-white rounded-xl text-sm font-bold cursor-pointer hover:bg-white/10 transition text-center"
                    >
                        Sign in
                    </button>
                </div>
